...
import {Brush, Info, Upload, ClearAll, Draw, Warning, CheckCircle} from "@mui/icons-material"
import "./Imagen2.css"
                ...
                <div className="assessment-table">
                  <table>
                    <thead>
                      <tr>
                        <th></th>
                        <th>policy violoation?</th>
                        <th>no</th>
                        <th>yes</th>
                      </tr>
                    </thead>
                    <tbody>
                      {Object.entries(prediction.assessment || {})
                        .sort(([policyA], [policyB]) => policyA.localeCompare(policyB))
                        .map(([policy, data]) => {
                          const yesProbability = data.probabilities?.[0] || 0;
                          const noProbability = data.probabilities?.[1] || 0;
                          const isViolation = yesProbability > 0.5;
                          return (
                            <tr 
                              key={policy} 
                              className={`table-row ${isViolation ? 'violation' : 'safe'}`}
                            >
                              <td className="decision-icon">
                                {isViolation ? (
                                  <Warning fontSize="small" className="warning-icon" />
                                ) : (
                                  <CheckCircle fontSize="small" className="safe-icon" />
                                )}
                              </td>
                              <td className="policy-name">{policy}</td>
                              <td className="prob-no">{(noProbability * 100).toFixed(1)}%</td>
                              <td className="prob-yes">{(yesProbability * 100).toFixed(1)}%</td>
                            </tr>
                          );
                        })}
                    </tbody>
...
function Imagen2() {
  ...
  const assessImage = async (prediction) => {
    try {
      const resizedImage = await resizeImage(prediction, {width: 512, height: 512})
      
      const assessmentBody = {
        instances: [resizedImage],
        parameters: {
          custom_policies: {
            "bc_high_school": "The image shall not contain content that is inappropriate for high school students (grades 9-12). This includes: graphic violence, gore, or depictions of serious injury; explicit sexual content or pornographic material; promotion or depiction of illegal activities (e.g., drug manufacturing, criminal acts); hate symbols or content promoting discrimination.",
            "bc_middle_school": "The image shall not contain content that is inappropriate for middle school students (grades 6-8). This includes: graphic violence, gore, or depictions of serious injury; explicit sexual content or pornographic material; promotion or depiction of illegal activities (e.g., drug manufacturing, criminal acts); hate symbols or content promoting discrimination; any nudity or sexual acts; promotion or depiction of dangerous activities (e.g., drug use, self-harm); bullying or harassment; extremely frightening or disturbing imagery.",
            "bc_elementary_school": "The image shall not contain content that is inappropriate for elementary school students (grades K-5). This includes: graphic violence, gore, or depictions of serious injury; explicit sexual content or pornographic material; promotion or depiction of illegal activities (e.g., drug manufacturing, criminal acts); hate symbols or content promoting discrimination; any nudity or sexual acts; promotion or depiction of dangerous activities (e.g., drug use, self-harm); bullying or harassment; extremely frightening or disturbing imagery; any depiction of violence, blood, or injury; any sexual content or suggestive imagery; scary, disturbing, or frightening scenes or characters; use of weapons; use of inappropriate language, gestures, or symbols."
          }
        }
      }

      const requestBody = JSON.stringify(assessmentBody)
      const requestSizeMB = (requestBody.length / (1024 * 1024)).toFixed(2)
            
      if (requestBody.length > 1.5 * 1024 * 1024) {
        console.warn(`Request size ${requestSizeMB}MB exceeds 1.5MB limit!`)
      }

      const res = await fetch("/api/assess-image", {
        method: "POST",
        headers: { 
      ...
      if (!res.ok) {
        console.error("Assessment failed:", await res.text())
        return null
      }
  ...
  const callApi = async (body) => {
    ...
    // Assess each generated image and add results to prediction objects
    const assessmentPromises = newItem.map(async (prediction) => {
      if (prediction.bytesBase64Encoded) {
        const assessment = await assessImage(prediction)
        console.log({ ...prediction, assessment: assessment.predictions[0] })
        return { ...prediction, assessment: assessment.predictions[0] }
      }
      return prediction
    })
    
    const assessedPredictions = await Promise.all(assessmentPromises)
    
    setHistory([assessedPredictions, ...history])
    setLoading(false)
    window.scroll({
      top: 0,
      left: 0,
  ...
  const resizeImage = (baseImage, {width, height, format = "image/png", quality = 1.0}) => {
